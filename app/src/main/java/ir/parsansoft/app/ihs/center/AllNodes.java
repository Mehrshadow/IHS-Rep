...
import ir.parsansoft.app.ihs.center.AllViews.CO_l_node_Thermostatic;
        ...
        public DiscoveryPacket(String packet) {
            try {
                ...
                if (p[0].equalsIgnoreCase("BHZDY")) {
                    ...
                    if (p[1].equalsIgnoreCase("SWTR"))
                        type = Node_Type.Thermostatic;
                } else {
                    type = 0;
                }
            } catch (Exception e) {
        ...
        public int addUI(View view) {
            ...
            if (switches.length >= 1) {
                ...
                if (switches[0].value == 0)
                    newSimpleKey.imgKey1.setImageResource(lay_simple_switch_off);
                else
            ...
            if (switches.length >= 2) {
                ...
                if (switches[1].value == 0)
                    newSimpleKey.imgKey2.setImageResource(lay_simple_switch_off);
                else
            ...
            if (switches.length >= 3) {
                ...
                if (switches[2].value == 0)
                    newSimpleKey.imgKey3.setImageResource(lay_simple_switch_off);
                else
                ...
                G.HANDLER.post(new Runnable() {
                    ...
                    public void run() {
                        for (int i = 0; i < simpleKeyUIs.size(); i++) {
                            ...
                            if (switches.length >= 1)
                                if (switches[0].value == 0) {
                                    simpleKeyUIs.get(k).imgKey1.setImageResource(lay_simple_switch_off);
                                } else {
                                    simpleKeyUIs.get(k).imgKey1.setImageResource(R.drawable.lay_simple_switch_on);
                                }
                            if (switches.length >= 2)
                                if (switches[1].value == 0) {
                                    simpleKeyUIs.get(k).imgKey2.setImageResource(lay_simple_switch_off);
                                } else {
                                    simpleKeyUIs.get(k).imgKey2.setImageResource(R.drawable.lay_simple_switch_on);
                                }
                            if (switches.length >= 3)
                                if (switches[2].value == 0) {
                                    simpleKeyUIs.get(k).imgKey3.setImageResource(lay_simple_switch_off);
                                } else {
...
//            ui.txtKey3.setText(switches[2].name); // Pump
            ...
            if (myNode.parentNodeId == 0) { // real key
                if (switches[0].value == 0) {
                ...
                if (switches[1].value == 0 && switches[2].value == 0) {
                    ui.imgKey1.setImageResource(R.drawable.lay_cooler_fast_off);
                    ui.imgKey2.setImageResource(R.drawable.lay_cooler_slow_off);
                } else if (switches[1].value == 1 && switches[2].value == 0) {
                    ui.imgKey1.setImageResource(R.drawable.lay_cooler_fast_off);
                    ui.imgKey2.setImageResource(R.drawable.lay_cooler_slow_on);
                } else if (switches[1].value == 0 && switches[2].value == 1) {
                    ui.imgKey1.setImageResource(R.drawable.lay_cooler_fast_on);
                    ui.imgKey2.setImageResource(R.drawable.lay_cooler_slow_off);
                }
            }
...
//                                    coolerUIs.get(k).imgKey1.setImageResource(R.drawable.lay_cooler_fast_off);
//                                    coolerUIs.get(k).imgKey2.setImageResource(R.drawable.lay_cooler_slow_on);
//                                    G.log("fast: off, slow: on");
//                                    // fast off
//                                } else if (switches[2].value == 1) {
//                                    coolerUIs.get(k).imgKey1.setImageResource(R.drawable.lay_cooler_fast_on);
//                                    coolerUIs.get(k).imgKey2.setImageResource(R.drawable.lay_cooler_slow_off);
//                                    G.log("fast: on, slow: off");
//                                    // fast on
//                                }
                ...
                G.HANDLER.post(new Runnable() {
                    ...
                    public void run() {
                        for (int i = 0; i < curtainUIs.size(); i++) {
                            ...
                            if (myNode.parentNodeId != 0) {
                                if (switches[0].value == 1) {// open bashe
                                ...
                                } else if (switches[1].value == 1) {// close bashe
                                ...
                                } else if (switches[2].value == 1) {// stop bashe
                                    curtainUIs.get(k).imgKey1.setImageResource(R.drawable.lay_curtain_open_off);
...
//        public int addUI(final CO_l_node_simple_key ui) {
        public int addUI(View view) {
            ...
            if (switches[0].value == 0) {
                ui.imgKey1.setImageResource(lay_simple_switch_off);// lamp off
            } else if (switches[0].value == 1) {
                ...
                G.HANDLER.post(new Runnable() {
                    ...
                    public void run() {
                        for (int i = 0; i < WCKeyUIs.size(); i++) {
                            ...
                            if (switches[0].value == 0) {
                                WCKeyUIs.get(k).imgKey1.setImageResource(lay_simple_switch_off);// lamp off
                            } else if (switches[0].value == 1) {
...
//            }
        ...
        public void switchKey(final int switchIndex, final float newValue, final NetMessage originalNetMessage, final LogOperator op, final int operatorID) {
            ...
            Database.Switch.Struct[] IOModuleSwitches = getIOModuleSwitches(myNode.iD);
        ...
        public static Database.Switch.Struct[] getIOModuleSwitches(int iD) {
            return Database.Switch.select("nodeID = " + iD + " isIOModuleSwitch = 1 ORDER BY IOModulePort ASC");
        }
...
//        public int addUI(final CO_l_node_simple_key newSimpleKey) {
    ...
    public static final class Thermostatic extends SampleNode {
        private SparseArray<CO_l_node_Thermostatic> termoUIs = new SparseArray<CO_l_node_Thermostatic>();
        private Database.Switch.Struct[] switches;
        private boolean isBusy;

        public Thermostatic(Database.Node.Struct node) {
            super(G.context);
            this.myNode = node;
            switches = select("NodeID=" + myNode.iD);
            if (myNode.parentNodeId == 0)
        ...
        public int addUI(View view) {
            ...
            G.log("This is adding Simple Node UI for node : " + myNode.iD);
            final CO_l_node_Thermostatic newKey = new CO_l_node_Thermostatic(view);
            newKey.prgDoOperation.setVisibility(View.INVISIBLE);
            switches = select("NodeID=" + myNode.iD);

            if (switches == null) {
                G.log("Switches count is ziro... could not add any UI");
                newKey.layKey1.setVisibility(View.INVISIBLE);
                newKey.layKey2.setVisibility(View.INVISIBLE);
                newKey.layKey3.setVisibility(View.INVISIBLE);
                return 0;
            }

            newKey.txtNodeName.setText(myNode.name);

            newKey.txtKey1.setText(switches[0].name);
            newKey.txtKey2.setText(switches[1].name);
            newKey.txtKey3.setText(switches[2].name);
            ...
            if (myNode.isFavorite)
                newKey.imgFavorites.setImageResource(R.drawable.icon_fav_full);
            else
                newKey.imgFavorites.setImageResource(R.drawable.icon_fav_empty);
            newKey.imgFavorites.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View arg0) {
                    myNode.isFavorite = !myNode.isFavorite;
                    Database.Node.edit(myNode);
            ...
            newKey.imgbtnEdit.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View arg0) {
                    Intent fw2 = new Intent(G.currentActivity, ActivityAddNode_w2.class);
                    fw2.putExtra("NODE_ID", myNode.iD);
            ...
            newKey.imgKey1.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View arg0) {
                    if (!isBusy)
                        switchKey(0, LogOperator.OPERAOR, G.currentUser.iD, setPoint[0]);
                }
            });

            newKey.imgKey2.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View arg0) {
                    if (!isBusy)
                        switchKey(1, LogOperator.OPERAOR, G.currentUser.iD, setPoint[0]);
                }
            });

            newKey.imgKey3.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View arg0) {
                    if (!isBusy)
                        switchKey(2, LogOperator.OPERAOR, G.currentUser.iD, setPoint[0]);
                }
            });

            newKey.btnSetPoint.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    DialogClass dlgInput = new DialogClass(G.currentActivity);
                    dlgInput.setOnOkCancelInputListener(new DialogClass.OkCancelInputListener() {

                        @Override
                        public boolean okClick(String input) {
                            if (input.length() == 0 || Integer.parseInt(input) > 37) {
                                new DialogClass(G.currentActivity).showOk(G.T.getSentence(865), G.T.getSentence(576));
...
//                                newKey.txtSetPoint.setText(input);
                    ...
                    dlgInput.showOkCancelInputNumerical(G.T.getSentence(865), G.T.getSentence(866));
                }
            });
        ...
        public void setProgressVisiblity(final boolean visiblity) {
            ...
            G.HANDLER.post(new Runnable() {
                ...
                public void run() {
                    for (int i = 0; i < termoUIs.size(); i++) {
                        if (visiblity == true) {
                            termoUIs.valueAt(i).prgDoOperation.setVisibility(View.VISIBLE);
                        } else {
                            termoUIs.valueAt(i).prgDoOperation.setVisibility(View.INVISIBLE);
                        }
                    }
                }
            });
        ...
        public void setSettingVisiblity(int uiIndex, boolean isVisible) {
            ...
            if (isVisible) {
                if (myNode.parentNodeId != 0)
                    termoUIs.get(uiIndex).imgbtnEdit.setImageResource(R.drawable.ic_io_node_setting);
                termoUIs.get(uiIndex).imgbtnEdit.setVisibility(View.VISIBLE);
            } else {
                termoUIs.get(uiIndex).imgbtnEdit.setVisibility(View.GONE);
            }
        }
        ...
        public void switchKey(int switchIndex, LogOperator op, int operatorID, int setPoint) {
            final NodeMsg nodeMsg = new NodeMsg();
            nodeMsg.msgType = NodeMsgType.MSG_DO_OPERATION;
            nodeMsg.node = myNode;
            int newValue;
    ...
    public static int AddNewNode(Database.Node.Struct newNode, int parentNodeId, boolean shouldNotifyNodeCreation, int sensorModel) {
        ...
        switch (newNode.nodeTypeID) {
            case Node_Type.SIMPLE_SWITCH_1: // Simple 1 switch Node
            ...
            case Node_Type.CURTAIN_SWITCH:
                ...
                if (parentNodeId != 0) {
                    ...
                    sw.code = "2";
                    sw.switchType = Switch_Type.CURTAIN_STATUS_Stop;
                    sw.name = G.T.getSentence(120503);
                    sw.isIOModuleSwitch = isIOModuleNode;
                    Database.Switch.insert(sw);
                } else {
                    sw.code = "0";
            ...
            case Node_Type.Thermostatic:
                ...
                for (int i = 0; i < 7; i++) { // 0 = low, 1 = medium, 2 = height, 4=, damaye mohit, 5= set point, 6= power, 7 = hot&cold
                    ...
                    sw.name = G.T.getSentence(120600 + (i + 1));
                    sw.isIOModuleSwitch = isIOModuleNode;

                    switch (i) {
                        case 0:
                            sw.switchType = Switch_Type.Thermo_SPEED_SLOW;
                            break;
                        case 1:
                            sw.switchType = Switch_Type.Thermo_SPEED_MEDIUM;
                            break;
                        case 2:
                            sw.switchType = Switch_Type.Thermo_SPEED_FAST;
                            break;
                        case 3:
                            sw.switchType = Switch_Type.Thermo_CurrentTemperature;
                            break;
                        case 4:
                ...
                nodeCommunication.allNodes.put(newNode.iD, new Thermostatic(newNode));
                break;
        }
