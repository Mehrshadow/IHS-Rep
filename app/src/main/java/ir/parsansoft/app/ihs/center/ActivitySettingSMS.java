            ...
            public void onDataReceive(String data, boolean cached) {
                ...
                try {
                    ...
                    if (resultID == 1) { // OK
                        ...
                        return;
                    } else if (resultID == 3) { //Mobile number not correct
                        new DialogClass(G.currentActivity).showOk(G.T.getSentence(739), G.T.getSentence(743));
                        return;
                    } else if (resultID == 4) { // low charge
                        new DialogClass(G.currentActivity).showOk(G.T.getSentence(739), G.T.getSentence(864));
                        return;
                    } else if (resultID == 5) { //Mobile not supported
//                        new DialogClass(G.currentActivity).showOk(G.T.getSentence(739), G.T.getSentence(743));
                        return;
                    } else { //Unknown Error
