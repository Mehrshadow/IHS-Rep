...
public class ActivityMyHouse extends ActivityEnhanced implements View.OnClickListener {
    ...
    private Database.Node.Struct[] nodes = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fo.btnDeleteIO.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                G.log("Going to delete the node :" + ioNode[0]);
                final DialogClass dlg = new DialogClass(G.currentActivity);
                // check if any of scenarios has used this node?
                final Database.Node.Struct[] ioNodes = Database.Node.select("ip='" + ioNode[0].iP + "'");
                for (int i = 0; i < ioNodes.length; i++) {
                    Database.Switch.Struct[] sws = Database.Switch.select("NodeID=" + ioNodes[i].iD);
                    if (sws != null) {
                        String swsIDs = "";
                ...
                dlg.setOnYesNoListener(new DialogClass.YesNoListener() {
                    ...
                    public void yesClick() {
                        ...
                        SysLog.log("Device :" + ioNode[0].name + " Deleted.", SysLog.LogType.DATA_CHANGE, SysLog.LogOperator.NODE, ioNode[0].iD);
                        Database.Node.Struct[] ioNodes = Database.Node.select("iP='" + ioNode[0].iP + "'");
